...
describe JavaBuildpack::Container::TcServerRedisStore do
  ...
  context do
    ...
    it 'detects with a session-replication service' do
      expect(component.detect).to eq("tc-server-redis-store=#{version}")
    end

    it 'copies resources',
       app_fixture:   'container_tc_server_redis_store',
       cache_fixture: 'stub-redis-store.jar' do

      component.compile

      expect(sandbox + "instance/lib/redis_store-#{version}.jar").to exist
    end
    ...
    it 'mutates context.xml',
      ...
      expect((sandbox + 'instance/conf/context.xml').read)
        .to eq(Pathname.new('spec/fixtures/container_tc_server_redis_store_context_after.xml').read)
    end
