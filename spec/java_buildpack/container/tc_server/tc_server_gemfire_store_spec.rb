...
require 'java_buildpack/container/tc_server/gemfire/gemfire'
require 'java_buildpack/container/tc_server/gemfire/gemfire_logging'
require 'java_buildpack/container/tc_server/gemfire/gemfire_logging_api'
require 'java_buildpack/container/tc_server/gemfire/gemfire_modules'
require 'java_buildpack/container/tc_server/gemfire/gemfire_modules_tomcat7'
require 'java_buildpack/container/tc_server/gemfire/gemfire_security'

describe JavaBuildpack::Container::TcServerGemfireStore do
  ...
  context do
    ...
    it 'mutates context.xml',
       app_fixture: 'container_tc_server_gemfire_store' do
      component.compile
      expect((sandbox + 'instance/conf/context.xml').read)
        .to eq(Pathname.new('spec/fixtures/container_tc_server_gemfire_store_context_after.xml').read)
    end

    it 'mutates server.xml',
       app_fixture: 'container_tc_server_gemfire_store' do
      component.compile
      expect((sandbox + 'instance/conf/server.xml').read)
        .to eq(Pathname.new('spec/fixtures/container_tc_server_gemfire_store_server_after.xml').read)
    end

    it 'creates cache-client.xml',
       app_fixture: 'container_tc_server_gemfire_store' do
      component.compile
      expect((sandbox + 'instance/conf/cache-client.xml').read)
        .to eq(Pathname.new('spec/fixtures/container_tc_server_gemfire_store_cache_client_after.xml').read)
    end
...
class StubGemfireStore < JavaBuildpack::Container::TcServerGemfireStore
